...
class commentsModelcomments extends igalleryModelbase
{
	...
	function __construct()
	{
		...
		$limitstart	= $mainframe->getUserStateFromRequest( $option.'.comments.limitstart', 'limitstart', 0, 'int' );
	...
	{
		if (empty($this->_commentsList))
		{
			$query = $this->_buildQuery();
			
			$this->_commentsList = $this->_getList($query, $this->getState('limitstart'), $this->getState('limit'));
		}
	...
	function _buildQuery()
	{
		...
		$query = 'SELECT * from #__igallery_comments '.$where.' '.$orderby;
		return $query;
	}
	...
	function publish($publish)
	{
		...
		$query = 'UPDATE #__igallery_comments SET published = '. $publish . ' WHERE id IN ( '.$cids.' )';
		$this->_db->setQuery($query);
		if ( !$this->_db->query() ) 
		{
	...
	{
		$cids = implode( ',', $this->_cid );
		$query = 'DELETE FROM #__igallery_comments WHERE id IN ( '.$cids.' )';
		$this->_db->setQuery($query);
		if(!$this->_db->query()) 
		{
			JError::raise(2, 500, $this->_db->getErrorMsg());
