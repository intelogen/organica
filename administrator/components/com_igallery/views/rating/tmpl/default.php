...
<thead>
	<tr>
		...
		<th width="30">
			<input type="checkbox" name="toggle" value="" onclick="checkAll(<?php echo count( $this->ratingList ); ?>);" />
		</th>
		...
		<th class="title" style="text-align: left;">
			<?php echo JHTML::_('grid.sort', JText::_( 'RATING' ), 'rating', $this->lists['order_Dir'], $this->lists['order'] ); ?>
		</th>
...
<?php
$k = 0;
$numberRatings = count($this->ratingList);
$db =& JFactory::getDBO();

for ($i=0; $i < $numberRatings; $i++)
{
	$row = &$this->ratingList[$i];
...
<input type="hidden" name="controller" value="rating" />
