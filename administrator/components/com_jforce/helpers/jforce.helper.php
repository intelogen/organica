...
class JForceHelper {
	...
	function sortArray($original,$field, $start = null)
        ...
        /**
        * JTPL HACK Description
        * Default link for tags have been altered as required by Maxim
        * 
        * @param array $tags
        * @param mixed $pid
        * @return string
        */
		function prepareTags($tags,$pid) {
			...
			for($i=0;$i<count($tags);$i++):
				$tag = $tags[$i];
                $phaseid = JRequest::getCmd("id");
                //$link = JRoute::_('index.php?option=com_jforce&view=search&layout=results&pid='.$pid.'&keyword='.$tag);    
                // JTPL HACK
                
                $link = JRoute::_('index.php?option=com_jforce&view=phase&task=application&pid='.$pid.'&layout='.$tag."&phase_id=".$phaseid);    			
