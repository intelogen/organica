...
class JForceModelMessage extends JModel {
	...
	function _buildWhere()
	{
        ...
        $where .= ' AND deleted_by <> '. (int) $user->id;
    ...
    * JTPL HACK
    * 
    * Delete MEssages in a inbox
    * 
    * @param mixed $messages_array
    */
    function delete_messages($messages_array){
        $user = JFactory::getUser();
        //mdie($messages_array);
        
        foreach($messages_array as $m){
            
            $query = "DELETE FROM #__jf_messages WHERE "            
                ."(".$this->_db->nameQuote("from")." = ".$this->_db->Quote($user->id)." OR "
                .$this->_db->nameQuote("to")." = ".$this->_db->Quote($user->id).") AND "
                .$this->_db->nameQuote("deleted_by")." <> ".$this->_db->Quote($user->id)." AND "
                .$this->_db->nameQuote("deleted_by")." <> ".$this->_db->Quote(0)." AND "
                .$this->_db->nameQuote("id")." = ".$this->_db->Quote($m)." LIMIT 1"
                ;
            ...
            $query = "UPDATE #__jf_messages SET ".$this->_db->nameQuote("deleted_by")." = ".$this->_db->Quote($user->id)." WHERE "
                ."(".$this->_db->nameQuote("from")." = ".$this->_db->Quote($user->id)." OR "
                .$this->_db->nameQuote("to")." = ".$this->_db->Quote($user->id).") AND "
                .$this->_db->nameQuote("deleted_by")." = ".$this->_db->Quote(0)." AND "
                .$this->_db->nameQuote("id")." = ".$this->_db->Quote($m)." LIMIT 1"
