...
class JforceModelProject extends JModel {
	...
	function save($data)
	{
		...
		// Set the image for the project
		$project->imagethumb = 'default.png';	
	...
	function autoAddPeople($project) {
		...
		for($i=0; $i<count($people); $i++) :
			$person = $people[$i];
			$projectrole = JTable::getInstance('Projectrole', 'JTable');
			$projectrole->load($person->projectrole);
			$cf = JTable::getInstance('Projectrolecf', 'JTable');
			$cf->bind($projectrole);
			$cf->id = '';
			$cf->uid = $person->uid;
			$cf->pid = $project->id;
			$cf->role = $projectrole->id;
			$cf->store();
		endfor;
	...
	function getProjectDetails($project) {
		...
		if($project->imagethumb):
			...
			$project->imagethumb = '<img src="'.JURI::base().'jf_projects/project_icons/thumbs/default.png" />';
		endif;
