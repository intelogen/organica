...
class JforceModelProject extends JModel {
	...
	function save($data)
	{
        ...
        // JTPL HACK
        // ignore the project starts on date. 
	...
	function getCompletedTasks($project) {
        ...
        # JTPL HACK
        # Task Changed to Checklist.
		$taskModel =& JModel::getInstance('Checklist','JForceModel');
		$tasks = $taskModel->getCountChecklists($project, true);
	...
	function getTotalTasks($project) {
        ...
        # JTPL Hack
		$taskModel =& JModel::getInstance('Checklist','JForceModel');
		$tasks = $taskModel->getCountChecklists($project);
    ...
    // JTPL HACK
    function get_product_category_for_phase(){
        $phases = array();
        // This definition is also made at the jforceuser plugin. So if any modification made on either location
        // has to be reflected on both locations        
        $phases[0] = "";
        
        // Syntax for the arrays : $phases[category_id] = $phase_name
        // make sure 0 is not the category id
        
        $phases[8] = "Phase 1 Intestinal / Digestive Cleanse";
        $phases[9] = "Phase 2 Yeast / Candida Cleanse";
        $phases[11] = "Phase 3 Parasite Cleanse";
        $phases[12] = "Phase 4 Liver Cleanse";
        $phases[13] = "Phase 5 Heavy Metal / Chemical Cleanse";
        $phases[14] = "Phase 6 Adrenal Testing & Balancing";
        $phases[15] = "Phase 7 Hormone Testing & Balancing";
        $phases[16] = "Phase 8 Maintenance Protocols";
        $phases[17] = "Phase 9 Rebuilding & Support";
        $phases[18] = "Phase 10 Exercise Program";
        
//        echo "<pre>";
//        print_r($this->getProject()->name);
//        return array_keys($phases,$this->getProject()->name);
        return array_search($this->getProject()->name,$phases);
