...
class JforceModelSearch extends JModel {
		...
		foreach($types as $type):
			...
			if($results):
				for($i=0;$i<count($results);$i++):
					$result = $results[$i];
					$result->link = JRoute::_('index.php?option=com_jforce&view='.$type.'&layout='.$type.'&pid='.$result->pid.'&id='.$result->id);	
					
					$result->$title = JForceHelper::prepareSearchContent( $result->$title, 200, $needle );
					$result->$text = JForceHelper::prepareSearchContent( $result->$text, 200, $needle );
					$result->tags = JForceHelper::prepareSearchContent( $result->tags, 200, $needle );
					foreach ($searchwords as $hlword) {
						$result->$title = eregi_replace( $hlword, "<span class=\"highlight\">\\0</span>", $result->$title);
						$result->$text = eregi_replace( $hlword, "<span class=\"highlight\">\\0</span>", $result->$text);		
						$result->tags = eregi_replace( $hlword, "<span class=\"highlight\">\\0</span>", $result->tags);		
					}					
