 ...
 *
 * @version     $Id: mod_events_latest.lib.php 1102 2008-05-22 06:11:59Z tstahl $
 * @package     Events
	...
	// an isolated name space from everythng else (I hope).
		...
		function displayLatestEvents($asHTML=true){
			...
			if (floatval($_VERSION->getShortVersion())>=1.5){
				...
				$jev_dispatcher = & JDispatcher::getInstance();
			}
			else {
				global $_MAMBOTS;
				$_MAMBOTS->loadBotGroup( 'content' );
				$jev_dispatcher =& $_MAMBOTS;
			}
			...
			if( $this->inccss && !$isloaded_css && $asHTML) {
				...
				global $_VERSION;
				if (floatval($_VERSION->getShortVersion())>=1.5){
					$cssHTML = '<link href="' . JURI::root() . 'modules/mod_events_latest/mod_events_latest.css" rel="stylesheet" type="text/css" />' . "\n";
			...
			$events = 0;
			$date	= mktime(0,0,0,$now_m,$now_d, $now_Y);
							...
							foreach($condtoken['data'] as $token) {
								...
								switch ($match){
									...
									case 'contact':
										...
										$jev_dispatcher->trigger( 'onPrepareContent', array( &$dayEvent, &$this->modparams, 0 ), true ); 
									...
									case 'content':  // Added by Kaz McCoy 1-10-2004
										...
										if (!isset($dayEvent->introtext)) $dayEvent->introtext=null;
										$results = $jev_dispatcher->trigger( 'onPrepareContent', array( &$dayEvent, &$this->modparams, 0 ), true ); 
