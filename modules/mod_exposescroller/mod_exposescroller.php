...
function escr_lbl( $IniVar) {
	...
	if (isset($expLocale) && is_file('../modules/mod_exposescroller/'.$expLocale.'/'.$expLocale.'.mod_exposescroller.ini')) {
		$IniFile = '../modules/mod_exposescroller/'.$expLocale.'/'.$expLocale.'.mod_exposescroller.ini';
	} else { // No translation file found for this language: use default en-GB
		$IniFile = "../modules/mod_exposescroller/en-GB/en-GB.mod_exposescroller.ini";
	}
...
//function get_componentId($ModuleName = "com_expose") {
	if($newJoom) {
		$exp_live_site = JURI::base();
		//$exp_live_site = $this->baseurl;
		$modPath = $exp_live_site . 'modules/mod_exposescroller/'; // J!1.5.x creates a module directory now, 1.0.x didn't
	} else {
		global $mosConfig_live_site;
		$exp_live_site = $mosConfig_live_site.'/';
		$modPath = $exp_live_site . 'modules/';
		$query = "SELECT id FROM #__menu WHERE published = 1 AND link = 'index.php?option=com_expose' ORDER BY link" ;
...
if (is_numeric($setPath)) {
	require_once( dirname(__FILE__).'/mod_exposescroller/xml.php' ); // Lib for searching/loading Expose xml content files
...
// not numeric: search in requested folder
	...
	require_once( dirname(__FILE__).'/mod_exposescroller/folder.php' ); // Lib for searcheng/loading files from path
...
echo "<script language='javascript' type='text/javascript' src='".$modPath."mod_exposescroller/continiousscroll.js'></script> \n"
	. "<script language='javascript' type='text/javascript' src='".$modPath."mod_exposescroller/scrollinit.js'></script> \n";
...
if (eregi('shadowbox', $setLinking)) {
	if (file_exists ('components/com_expose/expose/shadowbox/build/js/shadowbox.js')) { // If Expose installed
		echo "<link rel='stylesheet' type='text/css' href='components/com_expose/expose/shadowbox/build/css/shadowbox.css' /> \n";
		echo "<script language='javascript' type='text/javascript' src='components/com_expose/expose/shadowbox/build/js/lib/yui-utilities.js'></script> \n";
		echo "<script language='javascript' type='text/javascript' src='components/com_expose/expose/shadowbox/build/js/adapter/shadowbox-yui.js'></script> \n";
		echo "<script language='javascript' type='text/javascript' src='components/com_expose/expose/shadowbox/build/js/shadowbox.js'></script> \n";
	} else {
		echo "<link rel='stylesheet' type='text/css' href='".$modPath."mod_exposescroller/shadowbox/build/css/shadowbox.css' /> \n";
		echo "<script language='javascript' type='text/javascript' src='".$modPath."mod_exposescroller/shadowbox/build/js/lib/yui-utilities.js'></script> \n";
		echo "<script language='javascript' type='text/javascript' src='".$modPath."mod_exposescroller/shadowbox/build/js/adapter/shadowbox-yui.js'></script> \n";
		echo "<script language='javascript' type='text/javascript' src='".$modPath."mod_exposescroller/shadowbox/build/js/shadowbox.js'></script> \n";
	}
}
...
*/
	...
	if (eregi($setLinking, 'shadowboxnav'))
		echo "ExpScrollLoadEvent(function(){Shadowbox.init({continuous:true,displayCounter:false,loadingImage:'".$modPath."mod_exposescroller/shadowbox/images/loading.gif'});});\n";  //init ShadowBox
