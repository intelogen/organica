		...
		if(count($categories) > 0){
			...
			if($news = $db->loadObjectList()) {
				...
				foreach($news as $item) {		
                    ...
                    array_push($content_cid, 'null');
                    array_push($content_title, $item->title);
                    array_push($content_text, $item->text);
                    array_push($content_date, $item->date);
                    array_push($content_date_publish, $item->date_publish);
                    array_push($content_price, '');
                    array_push($content_discount_is_percent, '1');
                    array_push($content_discount_start, '');
                    array_push($content_discount_end, '');
                    array_push($content_tax, '');
                    array_push($content_cat_name, '');
                    array_push($content_manufacturer, $item->manufacturer);
                    array_push($content_manufacturer_id, $item->manufacturer_id);
                    $news_amount++;
				}
			}
			$sql_where2 = '';
		  ...
		  SELECT DISTINCT
			m.file_url AS file, 
			content.virtuemart_product_id AS tid
          FROM 
			#__virtuemart_products AS content
			LEFT JOIN
				#__virtuemart_product_medias AS pm 
				ON 
				pm.virtuemart_product_id = content.virtuemart_product_id
			LEFT JOIN
				#__virtuemart_medias AS m 
				ON 
				m.virtuemart_media_id = pm.virtuemart_media_id
          WHERE
			('.$sql_where2.')
          ORDER BY
			pm.ordering ASC
