...
class NSP_GK4_Helper {
	...
	function init($module, $params) {
		...
		// new GK4 v.3.2 options
		$this->config['vm_itemid'] = $params->get('vm_itemid', 9999); // Itemid of VM
        $this->config['vm_add_to_cart'] = $params->get('vm_add_to_cart', 0); // enable showing VM "add to cart" button
		$this->config['vm_out_of_stock'] = $params->get('vm_out_of_stock', 1); // enable showing out of stock products
		$this->config['vm_show_price_type'] = $params->get('vm_show_price_type', 'base'); // item price type
        $this->config['vm_show_price_with_tax'] = $params->get('vm_show_price_with_tax', 1); // enable showing tax in price
		$this->config['vm_shopper_group'] = $params->get('vm_shopper_group', -1); // Shopper groups for VM
		$this->config['vm_show_discount_amount'] = $params->get('vm_show_discount_amount', 0); // enable showing discount amount
		$this->config['vm_show_tax'] = $params->get('vm_show_tax', 0); // enable showing tax only
		$this->config['vm_display_type'] = $params->get('vm_display_type', 'text_price'); // text + price display type
			...
			for($i = 0; $i < count($this->content["ID"]); $i++) {	
				if($i < ($this->config['news_column'] * $this->config['news_rows'] * $this->config['news_full_pages'])) {
					...
					if($k2_mode == FALSE && $vm_mode == FALSE){
						...
						$news_k2store = $renderer->store_vm($this->config, $this->content['ID'][$i], $this->content['CID'][$i], $this->content['price'][$i], '', '', '', $this->content['discount_start'][$i], $this->content['discount_end'][$i], $this->content['tax'][$i], $this->content['manufacturer_id'][$i]);
					}			
					// PARSING PLUGINS
